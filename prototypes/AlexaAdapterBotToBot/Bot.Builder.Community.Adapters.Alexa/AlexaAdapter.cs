...
namespace Bot.Builder.Community.Adapters.Alexa
{
    public class AlexaAdapter : BotAdapter
    {
        ...
        private AlexaResponseBody CreateResponseFromLastActivity(IEnumerable<Activity> activities, ITurnContext context)
        {
            ...
            AddDirectivesToResponse(context, response, activity);
        ...
        {
            if (activity.Attachments?.FirstOrDefault(a => a.ContentType == "googleBasicCard") != null)
            {
                var basicCardAttachment =
                    activity.Attachments.First(a => a.ContentType == "alexaBasicCard");

                var basicCardJObject = JObject.FromObject(basicCardAttachment.Content);
                var basicCard = basicCardJObject.ToObject<AlexaCard>();
                response.Response.Card = basicCard;
            }
        ...
        private static void AddDirectivesToResponse(ITurnContext context, AlexaResponseBody response, Activity activity)
        {
            response.Response.Directives = context.AlexaResponseDirectives().Select(a => a).ToArray();

            if (activity.Attachments?.FirstOrDefault(a => a.ContentType == "alexaDirective") != null)
            {
                var basicCardAttachment =
                    activity.Attachments.First(a => a.ContentType == "alexaDirective");

                var basicCardJObject = JObject.FromObject(basicCardAttachment.Content);
                var basicCard = basicCardJObject.ToObject<DisplayRenderBodyTemplate1>();
                response.Response.Directives = new object[]
                {
                    new DisplayDirective() { Template = basicCard }
                };
            }
        }
